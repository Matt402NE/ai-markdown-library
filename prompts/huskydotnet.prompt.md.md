...
mode: agent
---
%% The original prompt came from https://github.com/SebastienDegodez/copilot-instructions/blob/main/.github/prompts/huskydotnet.prompt.md %%
...
5. **Validate Husky Installation**
   ...
   6. Switch to a temporary branch:

      ```bash
      git checkout -b temp-validation-branch
      ```

   7. Make a commit with an invalid commit message (should fail):
   ...
   8. Make a commit with a valid conventional message (should succeed):

      ```bash
      git commit -m "feat(test): add test file"
      ```

   9. Switch back to your original branch (replace `<original-branch>` with the name of your main branch, e.g. `main` or `master`):
